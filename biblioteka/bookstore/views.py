...
from .models import BookReservation, User, Book, Chat, DeleteRequest, Feedback
...
def registerView(request):
	if request.method == 'POST':
		...
		a.save()
		messages.success(request, 'Zarejstrowano pomyślnie')
		return redirect('home')
	else:
	    messages.error(request, 'Rejestracja nie powiodła się, spróbuj ponownie')
...
@login_required
def user_request_form(request):
	return render(request, 'user/delete_request.html')
...
def book_reservation(request):
	if request.method == 'POST':
		book_id = request.POST['book_reservation']
		current_user = request.user
		genre = Book.objects.get(id = book_id).genre
		username = current_user.username
		# user_request = username + "  chce wypożyczyć książkę o id " + book_id

		a = BookReservation(user = current_user, book_id = book_id, genre = genre)
		a.save()
		messages.success(request, 'Rezerwacja wysłana')
		if current_user.is_user == True:
			return redirect('user_request_form')
		else:
			return redirect('request_form')
	else:
	    messages.error(request, 'Rezerwacja nie została wysłana')
	    return redirect('request_form')
...
def send_feedback(request):
	if request.method == 'POST':
		...
		username = current_user.username
		feedback = username + " " + " pisze:  " + feedback

		a = Feedback(feedback=feedback)
		a.save()
		messages.success(request, 'Feedback został wysłany')
		return redirect('feedback_form')
	else:
	    messages.error(request, 'Feedback nie został wysłany')
	    return redirect('feedback_form')
...
def uabook(request):
	if request.method == 'POST':
		...
		a = Book(title=title, author=author, genre=genre, publisher=publisher, 
			desc=desc, cover=cover, uploaded_by=username, user_id=user_id)
		a.save()
		messages.success(request, 'Książka dodana pomyślnie')
		return redirect('publisher')
	else:
	    messages.error(request, 'Książka dodana pomyślnie')
...
def labook(request):
	if request.method == 'POST':
	    ...
	    messages.error(request, 'Wystąpił błąd podczas dodawania')
	    return redirect('llbook')
...
class LBookReservation(LoginRequiredMixin,ListView):
	...
	def get_queryset(self):
		return BookReservation.objects.order_by('-id')
...
def raport(request):
	...
	count= BookReservation.objects.annotate(num_genre = Count('genre'))

	slownik = {'naukowe': 1, 'przygodowe': 3, 'sci-fi': 5}
		
	

	return render(request, 'user/uabout.html', slownik)
